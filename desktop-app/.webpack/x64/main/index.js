...
const API_BASE = isProd ? 'http://146.83.198.35:1651/api' : process.env.ELECTRON_API_BASE_URL || 'http://localhost:3001/api';
const LAB_ID = process.env.ELECTRON_LAB_ID ? Number(process.env.ELECTRON_LAB_ID) : null;
...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    // Pantalla completa
    kiosk: true,
    // Modo kiosco estricto - BLOQUEA TODO
    frame: false,
    // Sin barra de título
    alwaysOnTop: true,
    // Mantener ventana siempre visible
    minimizable: false,
    // Deshabilitar minimizar hasta login
    closable: false,
    // Deshabilitar cerrar hasta login
    webPreferences: {
  ...
  mainWindow.loadURL(`file://${require('path').resolve(__dirname, '..', 'renderer', 'main_window', 'index.html')}`);
}
function createTray() {
  if (tray) return;
...
async function ensureDeviceRegistered() {
  try {
    ...
    const {
      data
    } = await axios.post(`${API_BASE}${DEVICES_PATH}`, payload);
...
ipcMain.handle('dispositivo:getInfo', async (_evt, {
  ...
  return {
    ...
    hostname,
    ip,
    labId: labId || LAB_ID || null,
    suggestedNumber,
    fixedNumber: persisted.deviceNumber ?? null
  };
});
ipcMain.handle('dispositivo:setFixedNumber', async (_evt, {
  deviceNumber
}) => {
  const saved = persistDeviceData({
    deviceNumber
  });
  return {
    ok: true,
    savedNumber: saved.deviceNumber
  };
});
...
ipcMain.handle('auth:login', async (_evt, {
  ...
  try {
    ...
    return {
      ok: false,
      message: e?.response?.data?.message || e?.message || 'No se pudo iniciar sesión'
    };
  }
});
