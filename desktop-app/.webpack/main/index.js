...
function createWindow() {
  const win = new BrowserWindow({
    ...
    show: true,
    webPreferences: {
      preload: 'C:\\Users\\luisf\\Desktop\\Gestion_Laboratorios_FACE\\desktop-app\\.webpack\\renderer\\main_window\\preload.js',
...
async function ensureDeviceRegistered() {
  ...
  const {
    data
  } = await axios.post(`${API_BASE}${DEVICES_PATH}`, {
    labId: Number(LAB_ID),
    ip,
    hostname,
    deviceId,
...
ipcMain.handle('dispositivo:ensureRegistered', async () => {
  try {
    const info = await ensureDeviceRegistered();
    return {
      ok: true,
      ...info
    };
    ...
    return {
      ok: false,
      message: e?.response?.data?.message || e?.message || 'Error registrando'
    };
  }
});
