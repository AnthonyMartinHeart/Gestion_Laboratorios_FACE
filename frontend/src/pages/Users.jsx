...
import ViewIcon from '../assets/ViewIcon.svg?url';
...
const Users = () => {
  ...
  // Unify selection for both tables
  ...
  const alumnos = useMemo(() => (
    users.filter(u => {
      ...
      // Solo incluir usuarios con rol "estudiante" que tengan email de alumnos
  ...
  const noAlumnos = useMemo(() => (
    users.filter(u => {
      const rol = u.rol ? u.rol.toLowerCase().trim() : '';
      
      // Excluir profesores (van a su propia tabla)
      if (rol === 'profesor') return false;
      
      // Excluir estudiantes (van a la tabla de alumnos)
      if (rol === 'estudiante') return false;
      
      // Incluir todos los demás roles: administrador, consultor, usuario
      return true;
    })
  ), [users]);
    ...
    <div className='users-container'>
      <div className='table-container'>
        <div className='top-table'>
          <h1 className='title-table'>Usuarios Del Sistema</h1>
        ...
        <Table
          ...
          selectableRows={true}
          selectedRows={dataUser.filter(u => noAlumnos.some(n => n.rut === u.rut))}
        />
      </div>
